...
import com.example.paymentservice.payment.adapter.out.persistent.exception.PaymentAlreadyProcessException
...
class R2DBCPaymentStatusUpdateRepository(
    ...
    override fun updatePaymentStatusToExecute(orderId: String, paymentKey: String): Mono<Boolean> {
        checkPreviousPaymentOrderStatus(orderId)
            ...
            .handle { paymentOrder, sink ->
                when (paymentOrder.second) {
                    ...
                    PaymentStatus.SUCCESS.name -> {
                        sink.error(PaymentAlreadyProcessException(message = "이미 처리 성공한 결제입니다.", status = PaymentStatus.SUCCESS))
                    }

                    PaymentStatus.FAILURE.name -> {
                        sink.error(PaymentAlreadyProcessException(message = "이미 처리 실패한 결제입니다.", status = PaymentStatus.FAILURE))
                    }
                }
            }
            .collectList()
    ...
    companion object {
        ...
        val UPDATE_PAYMENT_ORDER_STATUS_QUERY = """
            ...
            SET payment_order_status = :status, update_at = CURRENT_TIMESTAMP
            WHERE order_id = :orderId
        """.trimIndent()
